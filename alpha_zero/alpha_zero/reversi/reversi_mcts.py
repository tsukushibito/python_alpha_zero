...
from .reversi_state import ReversiState, ReversiAction
import numpy as np
...
class MctsNode:
    ...
    def __init__(self, state: GameState, policy: float, dual_network: ReversiDualNetwork):
        ...
        self.child_nodes: List[MctsNode] = None  # 子ノード群
        ...
        if not self.child_nodes:
            ...
            for action, policy in zip(self.state.allowed_actions, policies):
                self.child_nodes.append(
                    MctsNode(self.state.take_action(action),
                             policy,
                             self.dual_network))
            return value
